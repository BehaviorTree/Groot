...
#ifdef USING_ROS
void MainWindow::on_actionLoadRosparam_triggered()
{
    std::string tree, metamodel;
    _node_handle.param<std::string>("/behavior_tree/current_tree", tree, "");
    _node_handle.param<std::string>("/behavior_tree/current_metamodel", metamodel, "");

    if( tree.size() == 0)
    {
        qDebug() << "[/behavior_tree/current_tree] is empty ";
        return;
    }

    if( metamodel.size() == 0)
    {
        qDebug() << "[/behavior_tree/current_metamodel] is empty ";
    }
    else{
        NodeFactory::clear();
        NodeFactory::loadModelFromString( QString::fromStdString(metamodel), _main_scene->registry() );
    }

    QString xml_text = QString::fromStdString(tree);
    loadFromXML(xml_text);

    ui->selectMode->setValue( 1 );
}
#endif
